...
function App() {
  ...
  useEffect(() => {
    if (window.ethereum) {
      ...
      const provider = new WalletConnectProvider({
        infuraId: "YOUR_INFURA_PROJECT_ID" // Remplacez par votre ID de projet Infura
      });
  ...
  const createGame = async () => {
    try {
      ...
      await contract.methods.createGame().send({ from: accounts[0], gas: 3000000 });
      alert('Game created');
    } catch (error) {
      console.error(error);
  ...
  const registerPlayer = async () => {
    try {
      ...
      await contract.methods.registerPlayer(gameId, pseudo).send({ from: accounts[0], gas: 3000000 });
  ...
  const registerMultiplePlayers = async () => {
    try {
      ...
      await contract.methods.registerMultiplePlayers(gameId, pseudos).send({ from: accounts[0], gas: 3000000 });
  ...
  const registerBots = async () => {
    try {
      ...
      await contract.methods.registerBots(gameId, numBots).send({ from: accounts[0], gas: 3000000 });
  ...
  const setEliminationRange = async () => {
    try {
      ...
      await contract.methods.setEliminationRange(gameId, minEliminationCount, maxEliminationCount).send({ from: accounts[0], gas: 3000000 });
  ...
  const startRound = async () => {
    try {
      const accounts = await web3.eth.getAccounts();
      await contract.methods.startRound(gameId).send({ from: accounts[0], gas: 3000000 });
      alert('Round started');
    } catch (error) {
      console.error(error);
      alert('Error: ' + error.message);
  ...
  const closeRegistration = async () => {
    try {
      const accounts = await web3.eth.getAccounts();
      await contract.methods.closeRegistration(gameId).send({ from: accounts[0], gas: 3000000 });
